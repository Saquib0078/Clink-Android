...
public class ItemTaskCommentViewHolder extends RecyclerView.ViewHolder {
    ...
    public ItemTaskCommentViewHolder(ItemCommentBinding binding, Context context, CommentModel commentModel, String taskID, int position, ReplyCallbackListener replyCallbackListener, CommentDeleteCallbackListener commentDeleteCallbackListener) {
...
// Handle replies
        ...
        adapter = new TaskCommentAdapter(commentModelList, taskID, null, commentDeleteCallbackListener);
        binding.recyclerView.setAdapter(adapter);
    }

    private void deleteComment() {
        RetrofitClient.getInstance(context).getApiInterfaces().deleteCommentTask(taskID, commentModel.getCommentID()).enqueue(new PegaResponseManager(new PegaCallback((Activity) context) {
            @Override
    ...
    private void loadReplies(String commentID) {
        ...
        RetrofitClient.getInstance(context).getApiInterfaces().getTaskCommentReplies(taskID, commentID, size).enqueue(new PegaResponseManager(new PegaCallback((Activity) context, false) {
            @Override
            public void onSuccess(@Nullable JSONObject... data) {
