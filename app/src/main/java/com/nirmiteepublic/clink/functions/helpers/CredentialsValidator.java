...
public class CredentialsValidator {
    ...
    public boolean validateName(String name, boolean showToast) {
        if (name.length() < MIN_NAME_LENGTH) {
            if (showToast) {
                PegaAnimationUtils.showToast(activity, "Please Enter a Valid Name");
            }
            return false;
        }
        if (name.length() > MAX_NAME_LENGTH) {
            if (showToast) {
                PegaAnimationUtils.showToast(activity, "Name can Contain Maximum " + MAX_NAME_LENGTH + " Characters");
            }
            return false;
        }
        return true;
